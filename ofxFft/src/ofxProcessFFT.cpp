...
void ProcessFFT::setup(){
    ...
    graphMaxSize = 100; //approx 10sec of history at 60fps
...
//---------------------------------------------
void ProcessFFT::drawHistoryGraph(ofPoint pt, fftRangeType drawType,int threshold){
    ...
    switch (drawType) {
        ...
        case HIGH:
            ...
            break;
        case MAXSOUND:
            drawAvgGraphUnScaled(pt, graphMaxSound,ofColor(255, 100, 255,200),200-threshold);
            break;
        default:
...
//---------------------------------------------
void ProcessFFT::drawAvgGraphUnScaled(ofPoint pt, vector<float> values, ofColor _color,int threshold){
        ofEnableAlphaBlending();
        ofPushMatrix();
        ofFill();
        ofSetColor(_color);
   ...
   // cout<< "Delta: " << avgMaximum - prevAvgMaximum <<endl;
	...
	ofDrawLine(0,threshold , 200, threshold);
...
//---------------------------------------------
void ProcessFFT::drawDebug(int x,int y){
    ofPushMatrix();
	ofTranslate(x, y);
    ofDrawBitmapStringHighlight("Loudest Band: " + ofToString(loudestBand), 0,20);
    ofDrawBitmapStringHighlight("Curr. Max Sound Val: "+ ofToString(maxSound), 0,40);
    ofDrawBitmapStringHighlight("Super Low Avg: " + ofToString(superLowEqAvg), 0,60);
    ofDrawBitmapStringHighlight("Low Avg: " + ofToString(lowEqAvg), 0,80);
    ofDrawBitmapStringHighlight("Mid Avg: " + ofToString(midEqAvg), 0,100);
    ofDrawBitmapStringHighlight("High Avg: " + ofToString(highEqAvg), 0,120);
    ofDrawBitmapStringHighlight("Avg Max Sound: " + ofToString(avgMaxSoundOverTime), 0,140);
    ofDrawBitmapStringHighlight("Delta: " + ofToString(getDelta()), 0,160);
    ofDrawBitmapStringHighlight("Delta Shift Detected: " + ofToString(abs(getDelta())>(avgMaxSoundOverTime*.20)), 0,180);
